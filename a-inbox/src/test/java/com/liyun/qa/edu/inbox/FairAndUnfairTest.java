  ...
  private static class Worker extends Thread {
    ...
    public void run() {
      ...
      for (int i = 0; i < 3; i++) {
        lock.lock();
        try {
          System.out.println("当前是第"+ (i+1) +"次循环，第一次获得锁，Lock by [" + getName() + "], Waiting by " + ((MyReentrantLock) lock).getQueuedThreads());
          try{
            lock.lock();
            System.out.println("当前是第"+ (i+1) +"次循环，第二次获得锁，Lock by [" + getName() + "], Waiting by " + ((MyReentrantLock) lock).getQueuedThreads());
          }finally {
            lock.unlock();
          }
        } finally {
